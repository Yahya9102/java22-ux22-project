...
const Profile: NextPage<UserProps> = ({}) => {
  ...
  useEffect(() => {
    const fetchGames = async () => {
      try {
        ...
        // Retrieve selected games from sessionStorage if available
        const storedSelectedGames = sessionStorage.getItem("selectedGames")
  ...
  useEffect(() => {
    // Save selected games to sessionStorage
              ...
              (game: { name: string; image: string }, index: number) => (
                <div
                  className={`${styles.gameCard} ${
                    selectedGames.includes(game.name) ? styles.selectedGame : ""
                  }`}
                  key={index}
